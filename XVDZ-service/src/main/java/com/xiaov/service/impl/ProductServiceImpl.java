...
public class ProductServiceImpl extends BaseServiceImpl<Product> implements ProductService{
	...
	public List<Product> getProductByMutiType(PageRequest pageRequest,String types){
		...
		return dao.getEntitiestNotLazy(new Product(),new String[]{"productType","user"},eqs,pageRequest.getOffset(),pageRequest.getPageSize());
	}
	...
	public List<Product> designerPage(Product product) {
		...
		if(product.getUsId()!=null){
			...
			return dao.getEntitiestNotLazy(new Product(),new String[]{"productType","user"},eqs,product.getOffset(),product.getPageSize());
		}
	...
	public List<Product> search(SearchModel search) {
		...
		return dao.getEntitiestNotLazy(new Product(),new String[]{"productType","user"},criterions2,search.getOffset(),search.getPageSize());
	}

	public List<Product> hot(Product product) {
		Criterion [] criterions2={Restrictions.eq("isModule",0),Restrictions.eq("deleteFlag",0)};
		return dao.getEntitiestNotLazyWithOrder(new Product(),new String[]{"productType","user"},criterions2,0,8, Order.desc("pdtSaleCount"));
	}
	...
	public List<Product> moudule(Product product) {
		...
		return dao.getEntitiestNotLazyWithOrder(new Product(),new String[]{"productType","user"},criterions2,product.getOffset(),product.getPageSize(), Order.desc("pdtSaleCount"));
	}
}
