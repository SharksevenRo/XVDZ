	...
	public String compressPic() {
		try {
			...
			if (img.getWidth(null) == -1) {
				...
				BufferedImage resizedImg = new BufferedImage((int) newWidth, (int) newHeight,
						BufferedImage.TYPE_INT_RGB);

				Graphics2D resizedG = resizedImg.createGraphics();
				resizedImg = resizedG.getDeviceConfiguration().createCompatibleImage((int) newWidth, (int) newHeight,
						Transparency.TRANSLUCENT);
				resizedG.dispose();
				resizedG = resizedImg.createGraphics();
				Image from = tag.getScaledInstance((int) newWidth, (int) newHeight, tag.SCALE_AREA_AVERAGING);
				resizedG.drawImage(from, 0, 0, null);
				resizedG.dispose();
				FileOutputStream out = new FileOutputStream(outputDir + "\\" + outputFileName);
				// JPEGImageEncoder可适用于其他图片类型的转换
				JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);
				encoder.encode(resizedImg);
				out.close();
			}
